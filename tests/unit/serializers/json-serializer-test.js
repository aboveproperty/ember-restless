...
var Post, PostGroup, Tag, Person, Comment, Email;
...
module('Unit | Serializers | json serializer', {
    beforeEach(){
        ...
        this.application.register('model:email', Email);
...
test('has to load model with same resourceName like attribute', function (assert) {
    var data = {
            id: 1,
            name: 'bela',
            emails: [{id: 1, email: 'a@b.c'}, {id: 2, email: 'b@c.d'}]
        },
        model = Person.load(data);

    assert.equal(model.get('emails.firstObject.email'), 'a@b.c');
    assert.equal(model.get('emails.lastObject.email'), 'b@c.d');
});
