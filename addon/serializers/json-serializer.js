...
var JSONSerializer = Serializer.extend({
    ...
    deserialize: function (resource, data, hasMany) {
        ...
        if (data[key] && ( typeof data[key] === "object") && !hasMany) {
            data = data[key];
        }
    ...
    deserializeProperty: function (resource, prop, value) {
        var attrName = this.attributeNameForKey(resource.get('klassName'), prop);
    ...
    _keyForResource: function (resource) {
        Ember.assert("You need add a klassName for a model", Ember.isPresent(resource.get('klassName')));

        return resource.get('klassName');
